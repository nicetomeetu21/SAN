...
class MultiTioDatamodule(pl.LightningDataModule):
    def __init__(self, image_root, label_root, contour_label_root, shape_label_root, region_mask_root, train_data_list, test_data_list, batch_size,
        ...
        self.contour_label_root = contour_label_root
        ...
        for name in self.train_names:
            subject = tio.Subject(
                ...
                label=tio.LabelMap(os.path.join(self.label_root, find_full_name(self.image_root, name)),
                                   reader=image_reader),
                contour_label=tio.LabelMap(os.path.join(self.contour_label_root, find_full_name(self.image_root, name)),
                                   reader=image_reader),
...
class MultiTioDatamodule_2(pl.LightningDataModule):
    def __init__(self, image_root, label_root, shape_label_root, region_mask_root, train_data_list, test_data_list, batch_size,
                 num_workers, train_per_size, queue_length, samples_per_volume, **kwargs):
        super().__init__()
        self.batch_size = batch_size
        self.num_workers = num_workers
