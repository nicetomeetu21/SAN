...
class MsImageDis(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        self.downsample = nn.AvgPool3d(3, stride=(1,2,2), padding=1, count_include_pad=False)
    ...
    def _make_net(self):
        dim = self.dim
        cnn_x = []
        cnn_x += [Conv3dBlock(self.input_dim, dim, (3,4,4), (1,2,2), 1, norm='none', activation=self.activ, pad_type=self.pad_type)]
        for i in range(self.n_layer - 1):
            cnn_x += [Conv3dBlock(dim, dim * 2, (3,4,4), (1,2,2), 1, norm=self.norm, activation=self.activ, pad_type=self.pad_type)]
            dim *= 2
